...
/**
 * This class extends the CloudSimCore to enable network simulation in CloudSim. Also, it
 * disables all the network models from CloudSim, to provide a simpler simulation of
 * networking. In the network model used by CloudSim, a topology file written in BRITE
 * format is used to describe the network. Later, nodes in such file are mapped to
 * CloudSim entities. Delay calculated from the BRITE model are added to the messages send
 * through CloudSim. Messages using the old model are converted to the apropriate methods
...
public class CloudSim {
	...
	/**
	 * Initialises CloudSim parameters. This method should be called before creating any
	 * entities.
	 * <p>
	 ...
	 * @param cal starting time for this simulation. If it is <tt>null</tt>, then the time
	 * will be taken from <tt>Calendar.getInstance()</tt>
	...
	/**
	 * Starts the execution of CloudSim simulation. It waits for complete execution of all
	 * entities, i.e. until all entities threads reach non-RUNNABLE state or there are no
	 * more events in the future event queue.
	 * <p>
	 * <b>Note</b>: This method should be called after all the entities have been setup
	 * and added.
	 * @return the double
	 * @throws NullPointerException This happens when creating this entity before
	...
	public static double startSimulation() throws NullPointerException {
		...
		try {
			...
			throw new NullPointerException(
					"CloudSim.startCloudSimulation() :" + " Error - you haven't initialized CloudSim.");
		}
	}

	/**
	 * Stops Cloud Simulation (based on {@link Simulation#runStop()}). This should be only
	 * called if any of the user defined entities <b>explicitly</b> want to terminate
	...
	public static void stopSimulation() throws NullPointerException {
		try {
			...
			throw new NullPointerException(
					"CloudSim.stopCloudSimulation() : " + "Error - can't stop Cloud Simulation.");
		}
	}
	...
	/**
	 * Sends a request to Cloud Information Service (GIS) entity to get the list of all
	 * Cloud hostList.
	 * @return A List containing CloudResource ID (as an Integer object) or if a CIS
	 * entity hasn't been created before
	 * @pre $none
	 * @post $none
	 */
	...
	/**
	 * Initialise the simulation for stand alone simulations. This function should be
	 ...
	 * simulations since entities are automatically added to the simulation upon
	 * instantiation.
	...
	/**
	 * Internal method used to add a new entity to the simulation when the simulation is
	 * running. It should <b>not</b> be called from user simulations.
	 * @param e The new entity
	 */
	...
	/**
	 * Internal method used to run one tick of the simulation. This method should
	 * <b>not</b> be called in simulations.
	 * @return true, if successful otherwise
	 */
	...
	/**
	 * Internal method used to stop the simulation. This method should <b>not</b> be used
	 * directly.
	 */
	...
	/**
	 * Sets an entity's state to be waiting. The predicate used to wait for an event is
	 * now passed to Sim_system. Only events that satisfy the predicate will be passed to
	 * the entity. This is done to avoid unnecessary context switches.
	 * @param src the src
	 * @param p the p
	 */
	...
	/**
	 * Removes all events that match a given predicate from the future event queue returns
	...
	/**
	 * Internal method used to start the simulation. This method should <b>not</b> be used
	 * by user simulations.
	 */
	...
	/**
	 * Check if the simulation is still running. This method should be used by entities to
	 * check if they should continue executing.
	...
	/**
	 * This method is called if one wants to resume the simulation that has previously
	 * been paused.
	...
	/**
	 * Start the simulation running. This should be called after all the entities have
	 * been setup and added, and their ports linked.
	...
	public static double run() {
		...
		while (true) {
			...
			if (pauseAt != -1
					&& ((future.size() > 0 && clock <= pauseAt && pauseAt <= future.iterator().next().eventTime())
