...
public class Cloudlet {
	...
	/**
	 * The User or Broker ID. It is advisable that broker set this ID with its own ID, so
	 * that CloudResource returns to it after the execution.
	 **/
	private int userId;
	...
	/**
	 * The input file size of this Cloudlet before execution (unit: in byte). in byte =
	 * program + input data size
	 */
	...
	/**
	 * Start time of executing this Cloudlet. With new functionalities, such as CANCEL,
	 * PAUSED and RESUMED, this attribute only stores the latest execution time. Previous
	 * execution time are ignored.
	 */
	...
	/**
	 * Allocates a new Cloudlet object. The Cloudlet length, input and output file sizes
	 * should be greater than or equal to 1. By default this constructor sets the history
	 * of this object.
	 * @param cloudletId the unique ID of this Cloudlet
	 * @param cloudletLength the length or size (in MI) of this cloudlet to be executed in
	 * a PowerDatacenter
	 * @param cloudletFileSize the file size (in byte) of this cloudlet <tt>BEFORE</tt>
	 * submitting to a PowerDatacenter
	 * @param cloudletOutputSize the file size (in byte) of this cloudlet <tt>AFTER</tt>
	 * finish executing by a PowerDatacenter
	...
	public Cloudlet(final int cloudletId, final long cloudletLength, final int pesNumber, final long cloudletFileSize,
		...
		this(cloudletId, cloudletLength, pesNumber, cloudletFileSize, cloudletOutputSize, utilizationModelCpu,
				utilizationModelRam, utilizationModelBw, true);
		vmId = -1;
	 ...
	 * @return <tt>true</tt> if the ID has successfully been set or <tt>false</tt>
	 * otherwise.
	 */
	 ...
	 * This Cloudlet length is calculated for 1 Pe only <tt>not</tt> the total length.
	 * @param cloudletLength the length or size (in MI) of this Cloudlet to be executed in
	 * a CloudResource
	 ...
	 * @param netServiceLevel determines the kind of service this cloudlet receives in the
	 ...
	 * For example, this Cloudlet has a length of 500 MI and requires 2 PEs. This means
	 * each Pe will execute 500 MI of this Cloudlet.
	...
	/**
	 * Gets the history of this Cloudlet. The layout of this history is in a readable
	 * table column with <tt>time</tt> and <tt>description</tt> as headers.
	...
	/**
	 * Gets the length of this Cloudlet that has been executed so far from the latest
	 * CloudResource. This method is useful when trying to move this Cloudlet into
	 * different CloudResources or to cancel it.
	 * @return the length of a partially executed Cloudlet or the full Cloudlet length if
	 * it is completed
	 * @pre $none
	 * @post $result >= 0.0
	 */
	 ...
	 * @return <tt>true</tt> if this Cloudlet has finished execution, <tt>false</tt>
	 * otherwise
	 * @pre $none
	 * @post $none
	 */
	...
	/**
	 * Sets the length of this Cloudlet that has been executed so far. This method is used
	 * by ResCloudlet class when an application is decided to cancel or to move this
	 * Cloudlet into different CloudResources.
	...
	/**
	 * Sets the user or owner ID of this Cloudlet. It is <tt>VERY</tt> important to set
	 * the user ID, otherwise this Cloudlet will not be executed in a CloudResource.
	 ...
	 * NOTE: This method <tt>should</tt> be called only by a resource entity, not the user
	 * or owner of this Cloudlet.
	...
	/**
	 * Sets the execution start time of this Cloudlet inside a CloudResource. <b>NOTE:</b>
	 * With new functionalities, such as being able to cancel / to pause / to resume this
	 * Cloudlet, the execution start time only holds the latest one. Meaning, all previous
	 * execution start time are ignored.
	 ...
	 * @param wallTime the time of this Cloudlet resides in a CloudResource (from arrival
	 ...
	 * @return the Cloudlet status code as a string or <tt>null</tt> if the status code is
	 * unknown
	 * @pre $none
	 * @post $none
	 */
	...
	/**
	 * Gets the time of this Cloudlet resides in the latest CloudResource (from arrival
	 * time until departure time).
	 ...
	 * @param resId a CloudResource entity ID
	 * @return the length of a partially executed Cloudlet or the full Cloudlet length if
	 * it is completed or <tt>0.0</tt> if not found
	 * @pre resId >= 0
	 * @post $result >= 0.0
	 */
	...
	/**
	 * Gets the time of this Cloudlet resides in a given CloudResource ID (from arrival
	 * time until departure time).
	 * @param resId a CloudResource entity ID
	 * @return the time of this Cloudlet resides in the CloudResource or <tt>0.0</tt> if
	 * not found
	 * @pre resId >= 0
	 * @post $result >= 0.0
	 */
	 ...
	 * @return the finish or completion time of this Cloudlet or <tt>-1</tt> if not
	 * finished yet.
	 * @pre $none
	 * @post $result >= -1
	 */
