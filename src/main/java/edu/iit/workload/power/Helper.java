...
public class Helper {
	...
	public Helper(ResultDataRepository repository) {
	...
	public static List<PowerHost> createHostList(int hostsNumber) {
		...
		for (int i = 0; i < hostsNumber; i++) {
			...
			hostList.add(new PowerHostUtilizationHistory(i, new RamProvisionerSimple(Constants.HOST_RAM[hostType]),
					new BwProvisionerSimple(Constants.HOST_BW), Constants.HOST_STORAGE, peList,
					new VmSchedulerTimeSharedOverSubscription(peList), Constants.HOST_POWER[hostType]));
		}
		return hostList;
	}
	...
	public void printResults(PowerDatacenter datacenter, List<Vm> vms, double lastClock, String experimentName,
		...
		r.setNumberOfVms(numberOfVms);
		r.setTotalSimulationTime(totalSimulationTime);
		r.setSla(sla * 100);
		r.setNumberOfMigrations(numberOfMigrations);
