package edu.iit.workload.network.datacenter;
...
public class TestBagofTaskApp extends AppCloudlet {
	...
	public void createCloudletList(List<Integer> vmIdList) {
		// basically, each task runs the simulation and then data is consolidated in one
		// task
		...
		for (int i = 0; i < numbervm; i++) {
			...
			NetworkCloudlet cl = new NetworkCloudlet(NetworkConstants.currentCloudletId, executionTime / numbervm,
					pesNumber, fileSize, outputSize, memory, utilizationModel, utilizationModel, utilizationModel);
			...
			cl.stages.add(new TaskStage(NetworkConstants.EXECUTION, NetworkConstants.COMMUNICATION_LENGTH,
					executionTime / numbervm, stgId++, memory, vmIdList.get(0), cl.getCloudletId()));
			...
			if (i == 0) {
				for (int j = 1; j < numbervm; j++)
					cl.stages.add(new TaskStage(NetworkConstants.WAIT_RECV, NetworkConstants.COMMUNICATION_LENGTH, 0,
							stgId++, memory, vmIdList.get(j), cl.getCloudletId() + j));
			}
			else {
				cl.stages.add(new TaskStage(NetworkConstants.WAIT_SEND, NetworkConstants.COMMUNICATION_LENGTH, 0,
						stgId++, memory, vmIdList.get(0), t));
			}
